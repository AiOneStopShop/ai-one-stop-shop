...
export default function Newsletter() {
  ...
  const { subscribe, isLoading, error, success, clearMessages } = useSubscriberManagement()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await subscribe({
      ...
      interests: ['AI tools', 'technology', 'productivity'],
      source: 'newsletter'
    })
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="flex flex-col sm:flex-row gap-4 max-w-md mx-auto">
              ...
              <button 
                type="submit"
                disabled={isLoading}
                className="bg-white text-brand-purple font-medium py-3 px-6 rounded-lg hover:bg-gray-100 transition-colors whitespace-nowrap disabled:opacity-50 disabled:cursor-not-allowed"
              >
            ...
            {success && (
              <div className="text-green-200 text-sm max-w-md mx-auto">
                {success}
              </div>
            )}
          </form>

...
`;
  ...
  pages.forEach((page, index) => {
    ...
    if (links.length > 0) {
      const internalLinks = links.filter(link => link.url.includes(new URL(page.url).hostname));
      const externalLinks = links.filter(link => !link.url.includes(new URL(page.url).hostname));
  ...
  summary += `- Internal Links: ${allLinks.filter(link => link.url.includes(new URL(pages[0].url).hostname)).length}\n`;
  summary += `- External Links: ${allLinks.filter(link => !link.url.includes(new URL(pages[0].url).hostname)).length}\n`;
  ...
  if (imagesWithAlt && imagesWithAlt.length < allImages.length) {
...
async function main() {
  try {
    ...
    const sites = [
      { url: 'http://localhost:3002', name: 'AI One Stop Shop (Local Development)' },
    ...
    if (Object.keys(results).length > 1) {
      ...
      let comparison = `# AI One Stop Shop - Site Comparison Analysis
**Generated on:** ${new Date().toISOString()}

## 📋 **Comparison Summary**

| Site | Pages | Status | Analysis File |
|------|-------|--------|---------------|
`;

      Object.entries(results).forEach(([name, result]) => {
        comparison += `| ${name} | ${result.pagesCount} | ✅ Complete | \`${result.filename}\` |\n`;
      });
      
      comparison += `\n---\n## 🔍 **Key Differences**\n\n`;
      
      // Add comparison insights here
      comparison += `- **Local Development**: ${results['AI One Stop Shop (Local Development)']?.pagesCount || 0} pages\n`;
      comparison += `- **Live Site**: ${results['AI One Stop Shop (Live Site)']?.pagesCount || 0} pages\n`;
      comparison += `- **Content Synchronization**: ${Math.abs((results['AI One Stop Shop (Local Development)']?.pagesCount || 0) - (results['AI One Stop Shop (Live Site)']?.pagesCount || 0))} page difference\n`;
      
      const comparisonFilename = `Site_Comparison_Analysis_${new Date().toISOString().split('T')[0]}.md`;
      fs.writeFileSync(comparisonFilename, comparison);
      
      console.log(`📊 Comparison saved to: ${comparisonFilename}`);
    }

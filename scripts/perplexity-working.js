...
async function researchKeywordsWithPerplexity() {
  try {
    ...
    for (const research of researchQueries) {
      ...
      try {
        const response = await perplexity.languageModel({
...
function generateKeywordReport(results) {
  ...
  let report = `# AI One Stop Shop - Perplexity Keyword Research Report
...
async function main() {
  try {
    ...
    const filename = `AI_One_Stop_Shop_Perplexity_Keyword_Research_${timestamp}.md`;
    
    fs.writeFileSync(filename, report);
    
    // Step 4: Save raw data
    const rawDataFilename = `perplexity_keyword_research_raw_data_${timestamp}.json`;
